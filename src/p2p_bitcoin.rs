...
impl P2PBitcoin {
    ...
    pub fn start(&self, executor: &mut ThreadPool) {
        ...
        let height = if let Some(tip) = self.chain_db.read().unwrap().header_tip() {
        ...
        let (p2p, p2p_control) = P2P::new(p2pconfig, PeerMessageSender::new(sender), 10);

        let downstream = Arc::new(Mutex::new(BitcoinDriver {
            store: self.content_store.clone(),
        }));
        ...
        if self.discovery {
            dispatcher.add_listener(AddressPoolMaintainer::new(
                p2p_control.clone(),
                self.db.clone(),
                murmel::p2p::SERVICE_BLOCKS,
            ));
        }
        ...
        let dns = dns_seed(self.network);
        {
            ...
            let mut tx = db.transaction();
            for a in &dns {
                tx.store_address("bitcoin", a, 0, 0, 0)
                    .expect("can not store addresses in db");
            }
            tx.commit();
        }
...
impl Future for KeepConnected {
    ...
    fn poll(mut self: Pin<&mut Self>, _: &mut Context<'_>) -> Async<Self::Output> {
        if self.p2p.n_connected_peers() < self.min_connections {
            let choice;
            {
                self.p2p.connected_peers().iter().for_each(|a| {
                    self.earlier.lock().unwrap().insert(a.clone());
                });
                choice = self
                    .db
                    .lock()
                    .unwrap()
                    .transaction()
                    .get_an_address("bitcoin", self.earlier.clone())
                    .expect("can not read addresses from db")
            }
            if let Some(choice) = choice {
                ...
                let eligible = self
                    ...
                    .iter()
                    .cloned()
                    .filter(|a| !self.earlier.lock().unwrap().contains(&a))
                    .collect::<Vec<_>>();
                if eligible.len() > 0 {
...
impl AddressPoolMaintainer {
    pub fn new(
        ...
        let mut m = AddressPoolMaintainer {
            db,
        ...
        thread::Builder::new()
            .name("address pool".to_string())
            .spawn(move || m.run(receiver))
            .unwrap();
                        ...
                        for (last_seen, a) in &av {
                                ...
                                && a.services & self.needed_services == self.needed_services
                            {
                                if let Ok(addr) = a.socket_addr() {
                                    ...
                                    tx.store_address("bitcoin", &addr, 0, *last_seen as u64, 0)
                                        .unwrap();
                                }
                            }
                        }
