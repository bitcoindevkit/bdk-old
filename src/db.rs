...
use rusqlite::{Connection, OptionalExtension, ToSql, Transaction, NO_PARAMS};
    ...
    pub fn transaction(&mut self) -> TX {
        TX {
            tx: self
                .connection
                .transaction()
                .expect("can not start db transaction"),
        }
    }
}
...
impl<'db> TX<'db> {
    ...
    pub fn rollback(self) {
        self.tx
            .rollback()
            .expect("failed to roll back db transaction");
    }
    ...
    pub fn store_txout(
        ...
        funding: Option<(&PublicKey, &sha256::Hash, u16)>,
    ) -> Result<(), Error> {
        ...
        "#,
                &[&tx.txid().to_string() as &dyn ToSql, &serialize(tx)],
            )?;
        }
        Ok(())
    }

    pub fn read_unconfirmed(
        &self,
    ) -> Result<Vec<(bitcoin::Transaction, Option<(PublicKey, sha256::Hash, u16)>)>, Error> {
        let mut result = Vec::new();
        // remove unconfirmed spend
        let mut query = self.tx.prepare(
            r#"
        ...
        for r in query.query_map(NO_PARAMS, |r| {
            ...
            result.push((
                deserialize::<bitcoin::Transaction>(tx.as_slice())
                    .expect("can not deserialize stored transaction"),
                if let Some(publisher) = publisher {
                    Some((
                        PublicKey::from_slice(publisher.as_slice())
                            .expect("stored publisher in txout not a pubkey"),
                        sha256::Hash::from_hex(
                            std::str::from_utf8(id.unwrap().as_slice()).unwrap(),
                        )
                        .expect("stored id in txout not hex"),
                        term.unwrap(),
                    ))
                } else {
                    None
    ...
    pub fn read_processed(&mut self) -> Result<Option<sha256d::Hash>, Error> {
        ...
        "#,
                ...
                |r| {
                    Ok(
                        sha256d::Hash::from_hex(r.get_unwrap::<usize, String>(0).as_str())
    ...
    pub fn store_coins(&mut self, coins: &Coins) -> Result<(), Error> {
        ...
        for (outpoint, coin) in coins.confirmed() {
            let proof = proofs
                .get(&outpoint.txid)
                .expect("inconsistent wallet, missing proof");
            ...
            statement.execute(&[
                &outpoint.txid.to_string() as &dyn ToSql,
                &outpoint.vout,
                &(coin.output.value as i64),
                &coin.output.script_pubkey.to_bytes(),
                &coin.derivation.account,
                &coin.derivation.sub,
                &coin.derivation.kix,
                if tweak == "".to_string() {
                    &Null
        ...
        for (unconfirmed, _) in self.read_unconfirmed()? {
            if let Some(proof) = proofs
                .values()
                .find(|p| p.get_transaction().txid() == unconfirmed.txid())
            {
                self.tx.execute(
                    r#"
                ...
                "#,
                    &[
                        &proof.get_block_hash().to_string() as &dyn ToSql,
                        &proof.get_transaction().txid().to_string(),
                    ],
                )?;
            }
        }
    ...
    pub fn read_coins(&mut self, master_account: &mut MasterAccount) -> Result<Coins, Error> {
        ...
        for r in query.query_map::<(OutPoint, Coin, ProvedTransaction), &[&dyn ToSql], _>(
            NO_PARAMS,
            |r| {
                Ok((
                    OutPoint {
                        txid: sha256d::Hash::from_hex(r.get_unwrap::<usize, String>(0).as_str())
                            .expect("transaction id not hex"),
                    ...
                    serde_cbor::from_slice(r.get_unwrap::<usize, Vec<u8>>(9).as_slice())
                        .expect("can not deserialize stored proof"),
                ))
            },
        )? {
        ...
        for r in query.query_map(NO_PARAMS, |r| Ok(r.get_unwrap::<usize, Vec<u8>>(0)))? {
            let tx = deserialize::<bitcoin::Transaction>(r?.as_slice())
                .expect("can not deserialize stored transaction");
    ...
    pub fn store_account(&mut self, account: &Account) -> Result<usize, Error> {
        ...
        "#,
            &[
                ...
                &account.address_type().as_u32(),
                &account.sub_account_number(),
                &account.master_public().to_string(),
    ...
    pub fn read_account(
        ...
        "#,
            &[&account_number as &dyn ToSql, &sub],
            |r| {
                Ok(Account::new_from_storage(
                    AccountAddressType::from_u32(r.get_unwrap::<usize, u32>(0)),
                    account_number,
                    sub,
                    ExtendedPubKey::from_str(r.get_unwrap::<usize, String>(1).as_str())
                        .expect("malformed master public stored"),
                    serde_cbor::from_slice(r.get_unwrap::<usize, Vec<u8>>(2).as_slice())
                        .expect("malformed instantiated keys stored"),
                    0,
                    look_ahead,
                    network,
                ))
    ...
    pub fn get_an_address(
        &self,
        network: &str,
        other_than: Arc<Mutex<HashSet<SocketAddr>>>,
        ...
        let eligible = statement
            .query_map::<SocketAddr, _, _>(
                &[
                    &((now - BAN_TIME) as i64) as &dyn ToSql,
                    &network.to_string(),
                ],
                |row| {
                    let s = row.get_unwrap::<usize, String>(0);
                    let addr = SocketAddr::from_str(s.as_str())
                        .expect("address stored in db should be parsable");
                    Ok(addr)
                },
            )?
            .filter_map(|socket| match socket {
                Ok(a) => {
                    if !other_than.contains(&a) {
                        Some(a)
                    } else {
                        None
                    }
        ...
        Ok(Some(
            eligible[std::cmp::min(
                len - 1,
                thread_rng().sample::<f64, _>(Poisson::new(len as f64 / 4.0).unwrap()) as usize,
            )],
        ))
    }
}
...
pub fn init(config_path: &Path, coins: &Coins, master: &MasterAccount) {
    ...
    {
        let mut tx = db.transaction();
        tx.store_coins(coins)
            .expect("can not store new wallet's coins");
        tx.store_master(master)
            .expect("can not store new master account");
        tx.commit();
    }
}
