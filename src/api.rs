...
static CONTENT_STORE: Lazy<Arc<RwLock<Option<SharedContentStore>>>> =
    Lazy::new(|| Arc::new(RwLock::new(None::<SharedContentStore>)));
...
pub fn init_config(
...
) -> Result<Option<InitResult>, Error> {
    let mut config_path = PathBuf::from(work_dir);
    config_path.push(network.to_string());
    fs::create_dir_all(&config_path).expect(
        format!(
            "unable to create config_path: {}",
    ...
    if let Ok(_config) = config::load(&file_path) {
        ...
        // create new wallet
        let (mnemonic_words, deposit_address, wallet) =
            Wallet::new(network, passphrase, pd_passphrase);
...
pub fn start(work_dir: PathBuf, network: Network, rescan: bool) -> Result<(), Error> {
    ...
    match CONTENT_STORE.write() {
        ...
        Ok(mut cs) => {
            if cs.is_some() {
                ...
                let mut chain_db = ChainDB::new(chain_file_path.as_path(), network)
                    .expect("can not open chain db");
                ...
                let mut master_account = MasterAccount::from_encrypted(
                    hex::decode(config.encryptedwalletkey)
                        .expect("encryptedwalletkey is not hex")
                        .as_slice(),
                    ExtendedPubKey::from_str(config.keyroot.as_str())
                        .expect("keyroot is malformed"),
                    config.birth,
                );
                ...
                // load wallet from master account
                {
                    ...
                    let account = tx
                        .read_account(0, 0, network, config.lookahead)
                        .expect("can not read account 0/0");
                    master_account.add_account(account);
                    let account = tx
                        .read_account(0, 1, network, config.lookahead)
                        .expect("can not read account 0/1");
                    master_account.add_account(account);
                    let account = tx
                        .read_account(1, 0, network, 0)
                        .expect("can not read account 1/0");
                    master_account.add_account(account);
                    let coins = tx
                        .read_coins(&mut master_account)
                        .expect("can not read coins");
                ...
                let trunk = Arc::new(ChainDBTrunk {
                    chaindb: chain_db.clone(),
                });
                info!(
                    "Wallet balance: {} satoshis {} available",
                    bitcoin_wallet.balance(),
                    bitcoin_wallet.available_balance(trunk.len(), |h| trunk.get_height(h))
                );

                content_store = Arc::new(RwLock::new(
                    ContentStore::new(db.clone(), trunk, bitcoin_wallet)
                        .expect("can not initialize content store"),
                ));

                *cs = Option::Some(content_store.clone());

                p2p_bitcoin = P2PBitcoin::new(
                    config.network,
                    config.bitcoin_connections,
                    config.bitcoin_peers,
                    config.bitcoin_discovery,
                    chain_db.clone(),
                    db.clone(),
                    content_store.clone(),
                    config.birth,
    ...
    let mut thread_pool = ThreadPoolBuilder::new()
        .name_prefix("futures ")
        .create()
        .expect("can not start thread pool");
...
pub fn withdraw(
    ...
    fee_per_vbyte: u64,
    amount: Option<u64>,
) -> Result<WithdrawTx, Error> {
    let store = CONTENT_STORE.read().unwrap().as_ref().unwrap().clone();
    let withdraw = store
        .write()
        .unwrap()
        .withdraw(passphrase, address, fee_per_vbyte, amount);
    match withdraw {
        Ok((t, f)) => Ok(WithdrawTx::new(t.txid(), f)),
        Err(e) => Err(e),
    }
}
...
fn open_db(config_path: &Path) -> DB {
    ...
    let db = DB::new(db_path.as_path()).expect(
        format!(
            "Can't open DB {}",
            db_path.to_str().expect("can't get db_path")
        )
        .as_str(),
    );
    db
