...
use crate::api::{
    balance, deposit_addr, init_config, load_config, remove_config, start, stop, update_config,
    withdraw, BalanceAmt, InitResult, WithdrawTx,
};
...
#[no_mangle]
#[cfg(feature = "android")]
pub unsafe extern "C" fn Java_org_bdk_jni_BdkLib_initLogger(_: JNIEnv, _: JObject) {
...
#[no_mangle]
pub unsafe extern "C" fn Java_org_bdk_jni_BdkLib_loadConfig(
    env: JNIEnv,
    _: JObject,
    j_work_dir: JString,
    j_network: jint,
...
#[no_mangle]
pub unsafe extern "C" fn Java_org_bdk_jni_BdkLib_removeConfig(
    env: JNIEnv,
    _: JObject,
    j_work_dir: JString,
    j_network: jint,
...
#[no_mangle]
pub unsafe extern "C" fn Java_org_bdk_jni_BdkLib_updateConfig(
    env: JNIEnv,
    _: JObject,
    j_work_dir: JString,
    j_network: jint,
    ...
    j_bitcoin_discovery: jboolean,
) -> jobject {
    let work_dir = string_from_jstring(&env, j_work_dir);
    let work_dir = PathBuf::from(work_dir);
    let network = network_from_jint(j_network);

    let bitcoin_peers_length = env
        .get_array_length(j_bitcoin_peers)
        .expect("error get_array_length j_bitcoin_peers");

    let mut bitcoin_peers: Vec<SocketAddr> = Vec::new();

    for i in 0..(bitcoin_peers_length) {
        let bitcoin_peer = env
            .get_object_array_element(j_bitcoin_peers, i)
        ...
        let bitcoin_peer = env
            .get_string(bitcoin_peer)
            .expect("error env.get_string bitcoin_peer");
        let bitcoin_peer = bitcoin_peer.to_str().expect("error bitcoin_peer.toStr()");

        let bitcoin_peer_addr =
            SocketAddr::from_str(bitcoin_peer).expect("error SocketAddr::from_str(bitcoin_peer)");

        bitcoin_peers.push(bitcoin_peer_addr);
    }

    let bitcoin_connections =
        usize::try_from(j_bitcoin_connections).expect("usize::try_from(j_bitcoin_connections");
...
#[no_mangle]
pub unsafe extern "C" fn Java_org_bdk_jni_BdkLib_initConfig(
    env: JNIEnv,
    _: JObject,
    j_work_dir: JString,
    j_network: jint,
    j_passphrase: JString,
    j_pd_passphrase: JString,
) -> jobject {
    let work_dir = string_from_jstring(&env, j_work_dir);
    let work_dir = PathBuf::from(work_dir);
    let network = network_from_jint(j_network);
    ...
    let pd_passphrase = pd_passphrase
        .iter()
...
#[no_mangle]
pub unsafe extern "C" fn Java_org_bdk_jni_BdkLib_start(
    env: JNIEnv,
    _: JObject,
    j_work_dir: JString,
    j_network: jint,
...
#[no_mangle]
pub unsafe extern "C" fn Java_org_bdk_jni_BdkLib_stop(_: JNIEnv, _: JObject) {
    stop()
}

// Option<BalanceAmt> org.bdk.jni.BdkLib.balance()
#[no_mangle]
pub unsafe extern "C" fn Java_org_bdk_jni_BdkLib_balance(env: JNIEnv, _: JObject) -> jobject {
    match balance() {
        Ok(balance_amt) => {
...
#[no_mangle]
pub unsafe extern "C" fn Java_org_bdk_jni_BdkLib_depositAddress(
    env: JNIEnv,
    _: JObject,
) -> jobject {
...
#[no_mangle]
pub unsafe extern "C" fn Java_org_bdk_jni_BdkLib_withdraw(
    env: JNIEnv,
    _: JObject,
    j_passphrase: JString,
    j_address: JString,
...
fn j_optional_empty(env: &JNIEnv) -> jobject {
    ...
    let j_result = env
        ...
        .l()
        .expect("error converting Optional.empty() jvalue to jobject");
...
fn j_optional_init_result(env: &JNIEnv, init_result: InitResult) -> jobject {
    let mnemonic_words = env
        .new_string(init_result.mnemonic_words)
    ...
    let j_result = env
        .new_object(
            ...
            &[
                JValue::Object(mnemonic_words.into()),
                JValue::Object(deposit_address.into()),
            ],
        )
    ...
    let j_result = env
        .call_static_method(
            ...
            &[JValue::Object(j_result)],
        )
        .expect("error Optional.of(InitResult)")
        .l()
        .expect("error converting Optional.of() jvalue to jobject");
...
fn j_optional_balance_amt_result(env: &JNIEnv, balance_amt: BalanceAmt) -> jobject {
    ...
    let j_result = env
        .new_object("org/bdk/jni/BalanceAmt", "(JJ)V", &[bal, conf])
    ...
    let j_result = env
        ...
        .expect("error Optional.of(BalanceAmt)")
        .l()
        .expect("error converting Optional.of() jvalue to jobject");
...
fn j_optional_config(env: &JNIEnv, config: &Config) -> jobject {
    ...
    let j_bitcoin_peer_vec: Vec<JString> = config
        .bitcoin_peers
        .iter()
        .map(|s| s.to_string())
    ...
    let j_bitcoin_peer_arr: jobjectArray = env
        .new_object_array(
            i32::try_from(j_bitcoin_peer_vec.len()).unwrap(),
            env.find_class("java/lang/String")
                .expect("error env.find_class(String)"),
    ...
    for i in 0..(j_bitcoin_peer_vec.len()) {
        env.set_object_array_element(
            ...
            j_bitcoin_peer_vec[i].into(),
        )
        .expect("error set_object_array_element");
    }

    let j_bitcoin_connections: JValue = jint::try_from(config.bitcoin_connections)
        .expect("error converting bitcoin_connections to jint")
        .into();

    let j_bitcoin_discover: JValue = jboolean::try_from(config.bitcoin_discovery)
        .expect("error converting bitcoin_discovery to jboolean")
        .into();
    ...
    let j_result = env
        .new_object(
            ...
            &[
                j_network_enum_ordinal,
                JValue::Object(j_bitcoin_peer_arr.into()),
                j_bitcoin_connections,
                j_bitcoin_discover,
            ],
        )
...
fn j_optional_string(env: &JNIEnv, string: &String) -> jobject {
    ...
    let j_result = env
        ...
        .expect("error Optional.of(String)")
        .l()
        .expect("error converting Optional.of() jvalue to jobject");
