...
impl SendTx {
    pub fn new(
        ...
        let mut own_unconfirmed = HashMap::new();
        {
            ...
            for (t, _) in tx
                .read_unconfirmed()
                .expect("can not read unconfirmed transactions")
            {
                own_unconfirmed.insert(t.txid(), t);
            }
        }

        let mut txsender = SendTx {
            p2p,
            db,
        ...
        thread::Builder::new()
            .name("sendtx".to_string())
            .spawn(move || txsender.run(receiver))
            .unwrap();
    ...
    fn run(&mut self, receiver: PeerMessageReceiver<NetworkMessage>) {
        ...
        while let Ok(msg) = receiver.recv() {
            match msg {
                PeerMessage::Incoming(pid, msg) => match msg {
                    NetworkMessage::GetData(ref inv) => {
                        let txs = inv
                            .iter()
                            .filter_map(|i| {
                                if i.inv_type == InvType::Transaction {
                                    Some(i.hash)
                                } else {
                                    None
                                }
                        ...
                        if !txs.is_empty() {
                            let txs = txs
                                ...
                                .filter_map(|h| {
                                    if let Some(cached) = self.cache.get_mut(h) {
                                        self.p2p
                                            .send_network(pid, NetworkMessage::Tx(cached.clone()));
                                        None
                                    } else {
                                        Some(*h)
                                    }
                            ...
                            if !txs.is_empty() {
                                ...
                                for (t, _) in tx
                                    ...
                                    .expect("can not read unconfirmed transactions")
                                    .iter()
                                    .filter(|(t, _)| txs.contains(&t.txid()))
                                {
                                    self.p2p.send_network(pid, NetworkMessage::Tx(t.clone()));
                                    debug!(
                                        "sent our transaction {} at request of peer={}",
                                        t.txid(),
                                        pid
                                    );
                                }
                    ...
                    NetworkMessage::Inv(ref inv) => {
                        let have_not = inv
                            .iter()
                            .filter(|i| {
                                i.inv_type == InvType::Transaction
                                    && !self.cache.contains_key(&i.hash)
                            })
                            .cloned()
                            .collect::<Vec<_>>();
                        if !have_not.is_empty() {
                            self.p2p
                                .send_network(pid, NetworkMessage::GetData(have_not));
                        }
                    }
                    NetworkMessage::Tx(ref tx) => {
                        if self.cache.insert(tx.txid(), tx.clone()).is_none() {
                            self.p2p
                                .send_random_network(NetworkMessage::Inv(vec![Inventory {
                ...
                PeerMessage::Outgoing(msg) => match msg {
            ...
            if SystemTime::now()
                .duration_since(last_announcement)
                .unwrap()
                .as_secs()
                > 60
            {
                    ...
                    .expect("can not read unconfirmed transactions")
                {
                    if !self.cache.contains_key(&transaction.txid()) {
                        ...
                        {
                            debug!(
                                "announced our transaction {} to peer={}",
                                transaction.txid(),
                                peer
                            );
                        }
